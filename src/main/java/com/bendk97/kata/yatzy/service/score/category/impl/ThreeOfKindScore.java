package com.bendk97.kata.yatzy.service.score.category.impl;

import com.bendk97.kata.yatzy.model.DiceGroup.ThreeOfKind;
import com.bendk97.kata.yatzy.model.Die;
import com.bendk97.kata.yatzy.service.score.category.GroupByFunctions;

import java.util.Set;
import java.util.function.Function;


public final class ThreeOfKindScore extends AbstractGroupScore<ThreeOfKind> {

    @Override
    protected Function<Set<Die>, Set<ThreeOfKind>> groupByFunction() {
        return GroupByFunctions::getThreeOfKinds;
    }
}
